...
#[AsCommand(name: 'eesb:moneys5:stock-document-sample:transfer')]
final class MoneyS5TransferStockDocumentsSampleCommand extends ServiceBusCommandWithContext
{
    public function __construct(
        private readonly MoneyS5StockDocumentsSampleFacade $stockDocumentsSampleFacade,
    ...
    {
        $this
            ->setDescription('This command transfers new samples from MoneyS5')
    ...
    {
        $this->stockDocumentsSampleFacade->transferStockDocumentsSample($input->getArgument(self::CUSTOMER_CODE));
    }

    protected function failedMessage(InputInterface $input): string
    {
        return "Sync samples from MoneyS5 failed for customer {$input->getArgument(self::CUSTOMER_CODE)}";
    }

    protected function successfulMessage(): string
    {
        return 'All samples were transferred';
    }

    protected function context(): string
    {
        return 'elasticr.actions.service_bus.transfer_samples';
    }
}
